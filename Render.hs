...
data Scene = Scene
    ...
    , mObjects   :: [Object]
    , mLights    :: [Light]
    -- , mMaterials :: Map String Material
    } deriving (Eq, Show)
...
rayForPixel :: Pixel -> Ray
rayForPixel (x, y) = Ray (XYZ (fromIntegral x) (fromIntegral y) 0) unitZ
...
trace (Scene s w _ os ls) ray = trace' os ls ray (mDepth s) 1.0 (mSky w)

trace' :: [Object] -> [Light] -> Ray -> Int -> Double -> Colour -> Colour
trace' _ _ _ 0 _ colour = colour
trace' objs lights ray@(Ray _ d) level coef colour
    = case fstIntersection objs ray of
        ...
        Just (obj, t) -> let
            ...
            in trace' objs lights reflected (level - 1) (coef * mReflect mat)
